...
int main()
{
	...
	MDIO_vSetPortDir(DIO_PORTD , PORT_OUTPUT);		//portD LEDs
	...
	MDIO_vSetPinDir(SPI_PORT , SS , PIN_OUTPUT);	//set SS as output
	MDIO_vSetPinVal(SPI_PORT , SS , PIN_LOW);

	SPIconfig.MSTR_MODE = MASTER_MODE;
	...
	while (1)
	{
		MDIO_vSetPinVal(DIO_PORTB , SS , PIN_LOW);	//activate slave
		MSPI_vWrite(0xAB);
		MDIO_vSetPinVal(DIO_PORTB , SS , PIN_HIGH);	//deactivated slave
		Data = MSPI_uint8Read();
