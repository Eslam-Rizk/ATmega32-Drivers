...
// Global function pointers to store the address of the defined ISR functions
void (*EXTI0_CallBack)(void) = NULL;
void (*EXTI1_CallBack)(void) = NULL;
void (*EXTI2_CallBack)(void) = NULL;

// Callback functions to set the ISR function address for external interrupts 0, 1, and 2
void EXTI0_vSetCallBack(void (*pf)(void))
{
    EXTI0_CallBack = pf;
}
...
// Function to enable external interrupts and configure their sensing types
void MEXTI_vEnINT(EXTI_channel_t cpy_enumChannelNum)
{
    switch (cpy_enumChannelNum)
    {
    case EXT_INT0:
        // Configure sensing type for INT0
#if EXTI0_SENSE == LOW_LEVEL
        CLR_BIT(MCUCR, MCUCR_ISC00);
        CLR_BIT(MCUCR, MCUCR_ISC01);
#elif EXTI0_SENSE == ANY_CHANGE
...
#error "Invalid Sense Control"
    ...
    case EXT_INT1:
        // Configure sensing type for INT1
#if EXTI1_SENSE == LOW_LEVEL
        CLR_BIT(MCUCR, MCUCR_ISC10);
        CLR_BIT(MCUCR, MCUCR_ISC11);
#elif EXTI1_SENSE == ANY_CHANGE
...
#error "Invalid Sense Control"
    ...
    case EXT_INT2:
        // Configure sensing type for INT2
#if EXTI2_SENSE == RISING_EDGE
        SET_BIT(MCUCSR, MCUCSR_ISC2);
#elif EXTI2_SENSE == FALLING_EDGE
        CLR_BIT(MCUCSR, MCUCSR_ISC2);
...
void __vector_1(void)
{
    // Call the user-defined callback function for external interrupt 0 if assigned
    if (EXTI0_CallBack != NULL)
    {
        EXTI0_CallBack();
    }
...
void __vector_2(void)
{
    // Call the user-defined callback function for external interrupt 1 if assigned
    if (EXTI1_CallBack != NULL)
    {
        EXTI1_CallBack();
    }
...
void __vector_3(void)
{
    // Call the user-defined callback function for external interrupt 2 if assigned
    if (EXTI2_CallBack != NULL)
    {
        EXTI2_CallBack();
    }
